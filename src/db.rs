...
pub fn load_uid_mapping(path: &Path) -> Result<()> {
    ...
    let mut stmt = conn.prepare(&format!(
        "SELECT {UID_MAPPING_ENC}, {UID_MAPPING_QQ} FROM nt_uid_mapping_table"
    ))?;
...
pub fn list_sessions(limit: usize) -> Result<Vec<Session>> {
    ...
    // 私聊会话
    let mut stmt = conn.prepare(&format!(
        "SELECT {C2C_PEER_ID}, {GROUP_NAME}, MAX({TIMESTAMP})
         FROM c2c_msg_table
         WHERE {TIMESTAMP} > 0
         GROUP BY {C2C_PEER_ID}, {GROUP_NAME}
    ...
    // 群聊会话
    let mut stmt = conn.prepare(&format!(
        "SELECT {GROUP_NAME}, MAX({TIMESTAMP})
...
pub fn get_messages(
    ...
    if let Some(group_name) = chat.strip_prefix("group:") {
        ...
        let sql = format!(
            "SELECT {MSG_ID},{GROUP_SENDER_ID},{C2C_SENDER_NAME},{CONTENT},{TIMESTAMP},{IS_SENDER_ME}
             FROM dataline_msg_table
             WHERE {GROUP_NAME} = ? {} AND {CONTENT} IS NOT NULL
        ...
        if is_numeric {
            ...
            sql = format!(
                "SELECT {MSG_ID},{C2C_SENDER_ID},{C2C_SENDER_NAME},{CONTENT},{TIMESTAMP},{IS_SENDER_ME}
                 FROM c2c_msg_table
                 WHERE {C2C_PEER_ID} = ? {} AND {CONTENT} IS NOT NULL
                 ORDER BY {TIMESTAMP} DESC
                 LIMIT ? OFFSET ?",
                ts_where
            );
            ...
            sql = format!(
                "SELECT {MSG_ID},{C2C_SENDER_ID},{C2C_SENDER_NAME},{CONTENT},{TIMESTAMP},{IS_SENDER_ME}
                 FROM c2c_msg_table
                 WHERE {GROUP_NAME} LIKE ? {} AND {CONTENT} IS NOT NULL
                 ORDER BY {TIMESTAMP} DESC
                 LIMIT ? OFFSET ?",
                ts_where
            );
...
pub fn search_messages(
    ...
    // 群聊
    let sql2 = format!(
        "SELECT {MSG_ID},{GROUP_SENDER_ID},{C2C_SENDER_NAME},{CONTENT},{TIMESTAMP},{IS_SENDER_ME},{GROUP_NAME}
         FROM dataline_msg_table
         WHERE {CONTENT} IS NOT NULL {}
         ORDER BY {TIMESTAMP} DESC",
        ts_where
...
pub fn list_contacts(query: Option<&str>, limit: usize, kind: &str) -> Result<Vec<Contact>> {
    ...
    if kind == "all" || kind == "group" {
        if let Some(q) = query {
            ...
            let sql = format!("SELECT DISTINCT {GROUP_NAME} FROM dataline_msg_table LIMIT ?");
            let mut stmt = conn.prepare(&sql)?;
...
pub fn get_group_members(group_id: &str) -> Result<Vec<GroupMember>> {
    ...
    let sql = format!(
        "SELECT DISTINCT {GROUP_MEMBER_UID}, {C2C_SENDER_NAME}
         FROM dataline_msg_table
         WHERE {GROUP_NAME} = ? AND {GROUP_MEMBER_UID} IS NOT NULL AND {GROUP_MEMBER_UID} != 0"
    );
    ...
    let (c2c_count, group_count, since_str, until_str) = if let Some(chat) = chat {
        ...
        if let Some(group_name) = chat.strip_prefix("group:") {
            let count_sql = format!(
                "SELECT COUNT(*) FROM dataline_msg_table WHERE {GROUP_NAME} = ? {}",
                ts_where
            );
            let range_sql = format!(
                "SELECT MIN({TIMESTAMP}), MAX({TIMESTAMP})
                 FROM dataline_msg_table
                 WHERE {GROUP_NAME} = ? AND {TIMESTAMP} > 0 {}",
                ts_where
            );
            ...
            let peer_id = chat
                ...
                .with_context(|| format!("会话 ID 不是合法数字: {}", chat))?;
            let count_sql = format!(
                "SELECT COUNT(*) FROM c2c_msg_table WHERE {C2C_PEER_ID} = ? {}",
                ts_where
            );
            let range_sql = format!(
                "SELECT MIN({TIMESTAMP}), MAX({TIMESTAMP})
                 FROM c2c_msg_table
                 WHERE {C2C_PEER_ID} = ? AND {TIMESTAMP} > 0 {}",
                ts_where
            );
            let c2c_count = conn
            ...
            let count_sql = format!(
                "SELECT COUNT(*) FROM c2c_msg_table WHERE {GROUP_NAME} LIKE ? {}",
                ts_where
            );
            let range_sql = format!(
                "SELECT MIN({TIMESTAMP}), MAX({TIMESTAMP})
                 FROM c2c_msg_table
                 WHERE {GROUP_NAME} LIKE ? AND {TIMESTAMP} > 0 {}",
                ts_where
            );
            let c2c_count = conn
        ...
        let range_sql = format!(
            "SELECT MIN(ts), MAX(ts)
             FROM (
                 SELECT {TIMESTAMP} AS ts FROM c2c_msg_table WHERE {TIMESTAMP} > 0 {}
                 UNION ALL
                 SELECT {TIMESTAMP} AS ts FROM dataline_msg_table WHERE {TIMESTAMP} > 0 {}
             )",
            ts_where, ts_where
        );
        let (since_str, until_str) = conn
...
fn build_ts_where(since_ts: Option<i64>, until_ts: Option<i64>) -> String {
    ...
    if let Some(s) = since_ts {
        parts.push(format!("AND {TIMESTAMP} >= {}", s));
    }
    if let Some(u) = until_ts {
        parts.push(format!("AND {TIMESTAMP} <= {}", u));
    }
    parts.join(" ")
}
...
pub fn debug_tables() -> Result<()> {
    ...
    if tables.iter().any(|t| t == "c2c_msg_table") {
        ...
        let mut stmt = conn.prepare(&format!(
            "SELECT {MSG_ID},{TIMESTAMP},{IS_SENDER_ME},{CONTENT},[40900],[40600] FROM c2c_msg_table WHERE {CONTENT} IS NOT NULL LIMIT 10"
        ))?;
        let mut rows = stmt.query([])?;
        let mut count = 0;
        while let Some(row) = rows.next()? {
        ...
        let mut stmt = conn.prepare(&format!(
            "SELECT {MSG_ID},{TIMESTAMP},{C2C_SENDER_NAME},{CONTENT} FROM c2c_msg_table WHERE {CONTENT} IS NOT NULL AND length({CONTENT}) > 0 LIMIT 5"
        ))?;
        let mut rows = stmt.query([])?;
        let mut count = 0;
        while let Some(row) = rows.next()? {
...
pub fn debug_probe() -> Result<()> {
    ...
    // 找包含非 ASCII 字符的记录
    let mut stmt = conn.prepare(&format!(
        "SELECT {MSG_ID},{TIMESTAMP},{C2C_SENDER_NAME},[40090],[40093],{CONTENT} FROM c2c_msg_table LIMIT 10"
    ))?;
    let mut rows = stmt.query([])?;
    let mut count = 0;
    while let Some(row) = rows.next()? {
